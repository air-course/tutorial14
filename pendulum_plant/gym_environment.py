...
class SimplePendulumEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if state_representation == 2:
            ...
            self.low = np.array([-1., -1., -20.]) # changed from 8
    ...
    def reset(self, state=None, random_init=None):
        """
    ...
    def validation_criterion(self, validation_rewards,
        ...
        if np.mean(validation_rewards) > criterion:
            if goal_reached/(len(final_obs)*N) > 0.9:
                passed = True
