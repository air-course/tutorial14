...
class Simulator:
    ...
    def simulate(self, t0, x0, tf, dt, controller=None, integrator="runge_kutta", push_torque=0):
        """
        ...
        while self.t <= tf:
            if controller is not None:
                t0 = time.time()
                tau = controller.get_control_output(
            ...
            if self.t>5.0 and self.t<5.1 and push_torque!=0:
                self.step(push_torque, dt, integrator=integrator)
            else:
    ...
    def simulate_and_animate(
        ...
        text_plot = self.animation_ax.text(
            0.15, 0.85, [], fontsize=40*scale, transform=fig.transFigure
        )
        ...
        frames = [dict(par_dict) for _ in range(num_steps)]
